...
<?php if ( comments_open() ) : ?><div id="reviews"><?php
	...
	if ( have_comments() ) :
		...
		echo '<p class="add_review"><a href="#review_form" class="inline show_review_form button">'.__('Add Review', 'theretailer').'</a></p>';

		$title_reply = __('Add a review', 'theretailer');

	else :

		$title_reply = __('Be the first to review', 'theretailer').' &ldquo;'.$post->post_title.'&rdquo;';

		echo '<p>'.__('There are no reviews yet, would you like to <a href="#review_form" class="inline show_review_form">submit yours</a>?', 'theretailer').'</p>';
