		...
		<table width="100%" class="tribe-events-tickets">
			...
			foreach ( $tickets as $ticket ) {
				...
				if ( ( !$product->is_in_stock() ) || ( empty( $start_date ) || time() > $start_date ) && (  empty( $end_date ) || time() < $end_date ) ) {
					...
					echo "<td class='tickets_price single_variation'>";
					echo "<span class='price'>". $this->get_price_html( $product ) ."</span>";
					echo "</td>";
					/*
			...
			if ( $is_there_any_product ) {
				...
				<?php if ( $is_there_any_product_to_sell ) { ?>
					<tr>
						<td colspan="3" class='woocommerce'>

							<button type="submit"
							        class="single_add_to_cart_button button alt"><?php esc_html_e( 'Add to cart', 'tribe-wootickets' );?></button>
						</td>
					</tr>
					<?php
				}
